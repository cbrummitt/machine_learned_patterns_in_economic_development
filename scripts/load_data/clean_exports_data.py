...
import download_population_data as dlpop
...
def load_and_filter_CID_S2_trade_data(
    ...
    print(path_exports_data)
    print(path_exports_data.resolve())
    try:
...
def get_countries_with_small_population(
    ...
    """Filter countries with population `<= min_pop` in year
    `year_min_pop`
    """
...
def read_raw_data():
    return pd.read_csv(RAW_DATA_DIR / 'country_sitcproduct4digit_year.csv',
                       nrows=1000)  # TODO: remove


def main(product_code_digits=2):
    df_raw = read_raw_data()
    df_raw = df_raw.rename(columns={
        'sitc_product_code': 'product_code',
        'location_code': 'country_code',
        'export_value': 'export_value',
        'year': 'year',
    })

    start_year = df_raw.year.min()
    end_year = df_raw.year.max()

    population_data_world_bank = (
        dlpop.load_population_all_countries_from_world_bank(
            start_year, end_year))
    population_data_world_bank.year = (
        population_data_world_bank.year.astype(int))
    df = df_raw.merge(
        (population_data_world_bank
         .rename(columns={'iso3c': 'country_code'})
         .loc[:, ['country_code', 'year', 'population']]),
    ...
    exports_long_format = filter_exports(df, **filter_kws)

    exports_long_format = clean_missing_exports_long(
        exports_long_format, fill_na_value=0.0)
    ...
    exports_long_format.to_csv(CLEANED_DATA_DIR / 'exports_cleaned.csv')
